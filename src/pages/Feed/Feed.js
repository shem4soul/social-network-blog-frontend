...
const BACKEND_URL = "https://social-network-blog-backend.onrender.com";
...
class Feed extends Component {
  ...
  loadPosts = (direction) => {
    if (direction) this.setState({ postsLoading: true });
    ...
    axios
      .get(`${BACKEND_URL}/feed/posts?page=${page}`, {
        headers: { Authorization: "Bearer " + this.props.token },
      })
      .then((res) => {
        const posts = res.data.posts.map((post) => ({
          ...post,
          imagePath: post.imageUrl.startsWith("http")
            ? post.imageUrl
  ...
  updatePost = (post) => {
    this.setState((prevState) => {
      ...
      if (index > -1) updatedPosts[index] = post;
  ...
  deletePostHandler = (postId) => {
    ...
    axios
      .delete(`${BACKEND_URL}/feed/post/${postId}`, {
        headers: { Authorization: "Bearer " + this.props.token },
      })
      .then(() => this.loadPosts())
      .catch((err) => {
  ...
  newPostHandler = () => this.setState({ isEditing: true });

  startEditPostHandler = (postId) => {
    const editPost = { ...this.state.posts.find((p) => p._id === postId) };
    this.setState({ isEditing: true, editPost });
  };

  cancelEditHandler = () => this.setState({ isEditing: false, editPost: null });
